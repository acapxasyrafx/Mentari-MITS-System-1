...
export default {
  ...
  data () {
    return {
      ...
      activeTheme: this.$store.getters.paletteActive,
      textTheme: this.$store.getters.paletteText
    }
  },
  watch: {
    $route () {
    ...
    '$store.getters.paletteActive': function() {
      this.activeTheme = this.$store.getters.paletteActive;
    },
    '$store.getters.paletteText': function() {
      this.textTheme = this.$store.getters.paletteText;
    }
  },
  computed: {
    computedLinkClass () {
    ...
    computedLinkStyles () {
      ...
      if (this.isHovered || this.isActive) {
        style.backgroundColor = this.activeTheme

        if (this.contextConfig.gradient) {
          style.borderColor = this.isActive ? this.textTheme : 'transparent'
        } else {
          style.borderColor = colorShiftHsl(this.textTheme, {
            s: 13,
            l: -15,
          }).css
        }
    ...
    computedIconStyles () {
      if (this.contextConfig.invertedColor) {
        ...
        return {
          color: this.textTheme, //this.$themes.gray
        }
      }

      if (this.isHovered || this.isActive) {
        return {
          color: this.textTheme,
        }
      }

      return {
        color: this.textTheme,//'white',
      }
    },
  },
  methods: {
